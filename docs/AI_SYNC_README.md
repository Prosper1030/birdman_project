...
- **DSM 編輯器互動強化**：完成把手調整、橡皮筋框選與連線觸發最佳化
...
- **GUI 測試環境缺失**：PyQt5 測試缺少 libGL.so.1 導致無法啟動
- MCP 與現有後端服務整合仍需 PoC 以評估可行性

## 3. AI疑問/人工待回覆
- `pytest` 在 test_cli_rcpsp 出現 subprocess.CalledProcessError，需檢查 RCPSP CLI 執行環境
- `python -m src.gui_qt` 缺少 libGL.so.1 無法啟動
