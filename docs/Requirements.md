...
**從 WBS 創建 DSM 的視覺化流程**
系統除了支援直接匯入 DSM.csv 檔案外，新增視覺化創建流程：
- 使用者匯入 WBS 後可選擇進入「依賴關係編輯器」介面
- 提供從無到有的依賴關係建立工作流程

**視覺化依賴關係編輯器 (Visual Dependency Editor)**

**無限畫布介面設計**
- 參考 yEd 或 Heptabase 的白板概念，提供無限制的工作區域
- 支援縮放、平移、區域選取等基本畫布操作
- 提供網格對齊與磁吸功能輔助精確定位

**任務節點 (Task Nodes) 自動生成**
- WBS 中每個任務自動生成為獨立的視覺化節點（方塊或卡片形式）
- 節點內容顯示：Property 與 Name 組合（例如：「[A] 全機空氣動力學概念設計」）
- 節點顏色依 Property 分類自動配色，提供視覺化區分
- 支援節點樣式自訂：大小、顏色、邊框樣式

**直觀依賴關係建立**
- 滑鼠拖拉操作：從任務節點邊緣拖出箭頭連接到目標節點
- 箭頭方向表示依賴關係：A → B 代表 B 依賴 A（B 必須等 A 完成後才能開始）
- 支援多重依賴：一個任務可以依賴多個前置任務
- 即時依賴關係驗證：防止循環依賴的即時提醒

**自由佈局與自動整理功能**
- 自由拖拽：使用者可任意調整節點位置
- 自動佈局演算法選項：
  - 分層佈局 (Hierarchical Layout)：依據依賴關係自動分層排列
  - 正交佈局 (Orthogonal Layout)：最小化邊線交叉
  - 力導向佈局 (Force-Directed Layout)：基於物理模擬的自然排列
- 一鍵整理功能：「自動佈局」按鈕可瞬間優化所有節點位置

**DSM 矩陣反向生成**
- 視覺化編輯完成後，系統根據節點間箭頭連接關係自動生成標準 N×N DSM 矩陣
- 支援即時預覽：編輯過程中可隨時查看對應的 DSM 矩陣狀態
- 匯出功能：將生成的 DSM 矩陣儲存為標準 .csv 檔案格式
- 雙向同步：支援從 DSM 矩陣重新載入到視覺化編輯器
