...
from yed_edge_routing_merged import *
    ...
    def init_routing_system(self):
        ...
        try:
            ...
            try:
                ...
                # Some implementations expect router via manager
                if hasattr(self, 'edge_manager') and hasattr(self.edge_manager, 'router') and self.edge_manager.router is None:
                    self.edge_manager.router = YEdStyleEdgeRouter(self.scene.sceneRect(), grid_size=10.0)
        except Exception as e:
            print('[init_routing_system] fallback:', e)
...
def create_edge(self, src_id: str, dst_id: str):
    ...
    try:
        ...
        if getattr(self, "edge_manager", None):
    ...
    self.edges.append(edge)
    if hasattr(src_node, "add_edge"): src_node.add_edge(edge)
    if hasattr(dst_node, "add_edge"): dst_node.add_edge(edge)
    return edge
